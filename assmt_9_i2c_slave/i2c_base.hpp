...
class I2C_Base
{
    ...
    public:
        ...
        bool init_slave(uint8_t addr, uint8_t* slaveMem, size_t mem_size)
        {
            // Power on I2C
            ...
            // check memory addr and region whether it is valid addr or not
            mSlaveMem = slaveMem;
            mSlaveMemSize = mem_size;

            mpI2CRegs->I2ADR0 = addr;
            //mpI2CRegs->I2MASK0 = addr; not used in single address
            // sends ack response;

            // Enable I2C and the interrupt for it
            mpI2CRegs->I2CONSET = 0x44;// set EN and AA bit for slave mode
            NVIC_EnableIRQ(mIRQ);
            return true;
        }

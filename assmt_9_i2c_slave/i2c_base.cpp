...
I2C_Base::mStateMachineStatus_t I2C_Base::i2cStateMachine()
{
    enum {
        ...
        Slave_OwnAddrRecvdAckSent      = 0x60,
        Slave_RegOrDataRecvdAckSent       = 0x80,
        Slave_dataRecvdAndNAckSent  = 0x88,
        SpOrRsSentByMaster = 0xa0,
        SLA_rRecvdAckSent = 0xa8,
        dataAckedByMasterNextDataSent = 0xb8,
        NackedByMasterForNextData = 0xc0,
        LastDataSentAckedRecvdByMaster = 0xc8,
    ...
    {
        case Slave_OwnAddrRecvdAckSent:
        {
            // set STO, SI, AA(2) to zero to move to next state
            // set SI at the end
        ...
        case Slave_RegOrDataRecvdAckSent:
        {
            ...
            if(state_data == 1)
            {
                // already received the address index this time received the data
                ...
                {
                    //u0_dbg_printf("in nack\n");
                    setNackFlag(); // this has to check which state it will go after this state
                }
            }
            else// sub address and data both has been received
            {
                if(index < mSlaveMemSize) {
                    mSlaveMem[index] = reg_addr;
                index++;
                setAckFlag();
                }
                else// invalid index
        ...
        case Slave_dataRecvdAndNAckSent: // previously sent nack for last terminating communication
        {
            setAckFlag();
        ...
        case SpOrRsSentByMaster:// when master stops i2c transaction
        {// has to see what happens in the case when reset bit is set in master
            // set STO, SI, AA(2) to zero to move to next state
            // set SI at the end
                setAckFlag();
        ...
        case SLA_rRecvdAckSent:
        {
            // send data at the index requested by the master

                mpI2CRegs->I2DAT = mSlaveMem[index];
                index++;
                if(index < mSlaveMemSize)
                    setAckFlag();
        ...
        case dataAckedByMasterNextDataSent:
        {
            // master sent ack
            mpI2CRegs->I2DAT = mSlaveMem[index];
            index++;
            if(index == mSlaveMemSize) // last data byte sent
                setNackFlag();
            else
                setAckFlag();
        ...
        case NackedByMasterForNextData:
        {
            // master sent nack
            // has to see this condition works or not
            //setNackFlag();
            setAckFlag();
            clearSIFlag();
            break;
        }
        case LastDataSentAckedRecvdByMaster:
        {
            //setNackFlag();
            setAckFlag();
            clearSIFlag();

...
CMD_HANDLER_FUNC(i2cIoHandler)
{
    ...
    bool discover = cmdParams.beginsWithIgnoreCase("discover"); //send ack in slave side
...
// Add the below line into  handlers.hpp
/*
...
/* Adding the below line into "C:/SJSU_Dev/projects/lpc1758_freertos/L5_Application/source/terminal.cpp" file
//cp.addHandler(taskHandler, "task", "task1 and task2 operation")
*/
// assignment 9 cpu usage handler
#if 1
CMD_HANDLER_FUNC(taskHandler)
{
    extern TaskHandle_t taskhandle_1;
    extern TaskHandle_t taskhandle_2;
    output.putline("changing task status ");
    bool suspend = cmdParams.beginsWithIgnoreCase("suspend");
    bool resume = cmdParams.beginsWithIgnoreCase("resume");
    char task_name[6];
    // %*s is used to discard any length first string in scanf()
    cmdParams.scanf("%*s %s", &task_name); // will discard the first string and read second string
    output.printf("task_name %s ", task_name);
    if (suspend) {
        if (!strcmp(task_name, "task1"))
        {
            output.putline("suspending task1 ");
            vTaskSuspend(taskhandle_1);
            return true;
        }
        else if (!strcmp(task_name, "task2"))
        {
            output.putline("suspending task2 ");
            vTaskSuspend(taskhandle_2);
            return true;
        }
        else
        {
            output.putline("invalid task ");
            return false;
        }
    }
    if (resume) {
        if (!strcmp(task_name, "task1"))
        {
            output.putline("resuming task1 ");
            vTaskResume(taskhandle_1);
            return true;
        }
        else if (!strcmp(task_name, "task2"))
        {
            output.putline("resuming task2 ");
